...
// Weights are relative, not a budget that must total 100: finalize() renormalises
// across whichever categories could actually be measured. So adding Core Web
// Vitals leaves every existing score untouched until a CrUX key is configured.
export const CATEGORIES = [
  ...
  { key: 'vitals',    label: 'Core Web Vitals',      weight: 16 },
  { key: 'external',  label: 'External factors',     weight: 8  },
];

// Generative Engine Optimization. Evidence carries more weight than in the SEO
// model because the measured effect is larger: the GEO study found statistics,
// quotations and citations moved citation rates up to 40%, while the classic
// keyword levers did essentially nothing.
export const GEO_CATEGORIES = [
  { key: 'access',    label: 'AI crawler access',    weight: 24 },
  { key: 'answers',   label: 'Answer readiness',     weight: 26 },
  { key: 'evidence',  label: 'Evidence & citations', weight: 22 },
  { key: 'authority', label: 'Entity & authority',   weight: 16 },
  { key: 'structure', label: 'Machine readability',  weight: 12 },
];

export const CATEGORY_SETS = { seo: CATEGORIES, geo: GEO_CATEGORIES };

const POINTS = { ok: 1, warn: 0, fail: 0 };

export function createAudit(categoryDefs = CATEGORIES) {
  const categories = new Map(
    categoryDefs.map((c) => [c.key, { ...c, checks: [], measured: true, note: null }])
  );
  ...
  function finalize() {
    ...
    const todos = allFindings
      ...
      // Identical wording is the same task however many findings produced it;
      // sorting first means the surviving copy is the highest-priority one.
      .filter((f) => {
        const key = f.todo.trim();
