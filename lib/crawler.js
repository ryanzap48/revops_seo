...
import { IMPORTANCE } from './checks.js';
...
export async function crawlSite(seedUrl, opts = {}) {
  ...
  async function visit(item) {
    const audited = await analyze(item.url, { site });
  ...
  return {
    ...
    ...summarize(pages, { inbound, sitemapUrls, failures }),
    failures,
  };
}
...
function summarize(pages, { inbound, sitemapUrls, failures }) {
  ...
  // Category averages across every page that produced a score.
  const categoryKeys = ['meta', 'quality', 'structure', 'links', 'server', 'vitals', 'external'];
  const labels = {
    meta: 'Meta data', quality: 'Page quality', structure: 'Page structure',
    links: 'Link structure', server: 'Server configuration',
    vitals: 'Core Web Vitals', external: 'External factors',
  };
...
function aggregateTodos(pages) {
  ...
  for (const page of pages) {
    for (const todo of page.todos) {
      ...
      if (!groups.has(key)) {
        groups.set(key, { check: todo.check, category: todo.category, importance: todo.importance, pages: new Set(), actions: new Map() });
      }
  ...
  return [...groups.values()]
    .map((group) => ({
      ...
      sampleUrls: [...group.pages].slice(0, 5),
      // The most frequently produced wording is the most representative fix.
      action: [...group.actions.entries()].sort((a, b) => b[1] - a[1])[0][0],
    }))
    .sort((a, b) => {
